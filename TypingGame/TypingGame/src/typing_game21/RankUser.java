...
public class RankUser {
	...
	public RankUser(int difficulty) { //레벨별 파일정보 불러오기
		switch(difficulty) {
		case 1:
		...
		try {
			FileReader fileReader = new FileReader(txtFileName); //파일읽기
			int c;
			StringBuffer sb = new StringBuffer();
			while((c = fileReader.read()) != -1) {
				if(c == '\n') {
					String[] data = sb.toString().split(" ");
					//data[0]: 이니셜, data[1]: 점수
	...
	public void listSort() { //점수 순으로 배열정렬
	...
	public ArrayList<UserData> getList(){ return userList; }
	...
	public UserData getUser(int index) { return userList.get(index); }
	
	public String getRankFileName() { return txtFileName;}
}
